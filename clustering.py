...
def unnormalized_spectral_clustering(W,k):
 ...
 #    plt.plot(eigenvalues[index_sorted]);plt.show()
    k_first_eigenvectors = eigenvectors[:,index_sorted[:k]]
    kmeans = KMeans(n_clusters=k).fit(np.transpose(k_first_eigenvectors.T))
...
def normalized_laplacian_rw(W):
    d = np.sum(W,axis=1)
    D_inv = np.diag(1/d)
    return np.identity(W.shape[0]) - D_inv* W 

def normalized_laplacian_sym(W):
    d = np.sum(W,axis=1)
    D_inverse_root = np.diag(1/np.square(d))
    return np.identity(W.shape[0]) - D_inverse_root*W*D_inverse_root
...
def normalized_spectral_clustering(W,k):
    ...
    index_sorted = np.argsort(eigenvalues)
    k_first_eigenvectors = eigenvectors[:,index_sorted[:k]]
    kmeans = KMeans(n_clusters=k).fit(np.transpose(k_first_eigenvectors))
    prediction = kmeans.labels_
    clusters = {}
    for i in range(k):
        clusters[i] = np.where(prediction == i)
...
if __name__ == "__main__":
    ...
    W_neighbors = graph.k_nearest_neighbors_graph(S,10)
    
    #Unnormalized laplacian 
    eigenvalues, eigenvectors, clusters = unnormalized_spectral_clustering(W_neighbors,4)
    ...
    for i in clusters:
        ...
        plt.scatter(range(len(cluster)), data[cluster], c=colors[i])
    plt.show()